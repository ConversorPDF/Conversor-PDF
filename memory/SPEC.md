...
- `/api/tools/convert-video` — campo `files` (≥1 vídeo) + `target` (mp4|mov|avi|mkv) + opcionales `scale` (original|2160|1440|1080|720|480|360), `quality` (ligera|recomendada|maxima → CRF 20/23/28), `start`/`end` (recorte, `-ss`/`-to` antes de `-i`) → vídeo o ZIP. 4 tarjetas UI (Convertir, Cambiar resolución, Comprimir, Recortar). libx264+aac. Requiere ffmpeg.
- `/api/tools/extract-frames` — campo `file` (vídeo) + `interval` (seg, `fps=1/N`) + `fmt` (jpg|png) → ZIP de imágenes. Requiere ffmpeg.
